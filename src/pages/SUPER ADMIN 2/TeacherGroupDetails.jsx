...
import { groupsAPI, homeworkAPI, lessonsAPI, filesAPI, examAPI, attendanceAPI, parseApiError, fetchHomeworkByGroup, unwrapHomeworkResults, normalizeHomeworkResultStatus, parseFilesList, loadVideoForPlayback } from '../../api/api';
...
const TeacherGroupDetails = () => {
  ...
  const openHomeworkUrl = (realId) => {
    try {
      navigate(`/super-admin-2/groups/${id}/homework/${realId}/results?resultsTab=ACCEPTED`, { replace: false });
    } catch (e) {
  ...
  const closeHomeworkUrl = () => {
    ...
    try {
      navigate(`/super-admin-2/groups/${id}?tab=1`, { replace: true });
    } catch (e) {
      setSearchParams({ tab: String(TAB_MAP['darsliklar']) });
    }
  };
  ...
  useEffect(() => {
    ...
    if (!homeworkId || String(homeworkId).toLowerCase() === 'null' || String(homeworkId).toLowerCase() === 'undefined') {
      console.warn('[GroupDetails] invalid homeworkId route param, normalizing to group view:', homeworkId);
      try {
        navigate(`/super-admin-2/groups/${id}?tab=1`, { replace: true });
      } catch (e) {
        setSearchParams({ tab: String(TAB_MAP['darsliklar']) });
  ...
  useEffect(() => {
    const fetchGroupDetails = async () => {
      setIsLoading(true);
      try {
        // Teacher endpointlarini ishlatamiz - getById (getOne emas)
        // groupsAPI.getStudents — admin endpoint, teacher uchun 403 beradi
        // Shuning uchun groupsAPI.getById bilan guruh ichidagi students ni olamiz
        const [groupRes, scheduleRes] = await Promise.allSettled([
      ...
      {month.days.map((d, idx) => {
        ...
        return (
          <div
            ...
            onClick={() => { if (isClickable) navigate(`/groups/${id}/lesson?date=${isoDate}`); }}
            onKeyDown={(e) => { if (isClickable && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); navigate(`/groups/${id}/lesson?date=${isoDate}`); } }}
            title={
              done
                            ...
                            <tr key={item.id || idx}>
                              ...
                              <td
                                ...
                                onClick={() => {
                                  ...
                                  navigate(`/groups/${id}/homework/${realId}/results?resultsTab=ACCEPTED`, { replace: false });
                                }}
                              >
                                {hasPending ? (
                                  <span style={{
                              ...
                              <td style={{ textAlign: 'center', color: '#6b7280' }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                                  <FiUsers size={16} />
                                  <span>{stats.students > 0 ? stats.students : '-'}</span>
                                </div>
                              </td>
